...
#ifdef RW_NULL
// =======================================================
// 🎮 GameCube 物理外挂：强行补全 NULL 平台的顶点结构体及方法
// =======================================================
namespace rw {
    namespace null {
        struct Im2DVertex {
            ...
            float32 u, v;
            uint32 color;
            void setScreenX(float32 _x) { x = _x; }
            void setScreenY(float32 _y) { y = _y; }
            void setScreenZ(float32 _z) { z = _z; }
            void setCameraX(float32 _x) { }
            void setCameraY(float32 _y) { }
            void setCameraZ(float32 _z) { z = _z; }
            void setRecipCameraZ(float32 _w) { w = _w; }
            // RGBA packing must match gxim2d.cpp submitVertex (reads c>>24=R, c>>16=G, c>>8=B, c=A)
            void setColor(uint8 r, uint8 g, uint8 b, uint8 a) { color = ((uint32)r<<24)|((uint32)g<<16)|((uint32)b<<8)|(uint32)a; }
            void setIntRGBA(uint8 r, uint8 g, uint8 b, uint8 a) { setColor(r,g,b,a); }
            void setU(float32 _u, float32 _w) { u = _u; }
            void setV(float32 _v, float32 _w) { v = _v; }
        };
        ...
        struct Im3DVertex {
            ...
            uint32 color;
            float32 u, v;
            void setPos(float32 _x, float32 _y, float32 _z) { position.x=_x; position.y=_y; position.z=_z; }
            void setColor(uint8 r, uint8 g, uint8 b, uint8 a) { color = ((uint32)r<<24)|((uint32)g<<16)|((uint32)b<<8)|(uint32)a; }
            void setRGBA(uint8 r, uint8 g, uint8 b, uint8 a) { setColor(r,g,b,a); }
            void setU(float32 _u) { u = _u; }
            void setV(float32 _v) { v = _v; }
        };
    }
}
#endif
