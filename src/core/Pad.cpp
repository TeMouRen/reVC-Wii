...
GcScaleStickAxis(s8 value, bool invert)
{
	...
	if (Abs(scaled) <= deadzone)
		...
		scaled = (scaled - deadzone) * 128 / (fullScale - deadzone);
	else
		scaled = (scaled + deadzone) * 128 / (fullScale - deadzone);
...
static inline bool
GcActionDown(const CPad *pad, GcPadAction action)
{
...
static void
GcFinishPadFrame(CPad *pad)
{
	if(pad->NewState.CheckForInput())
		pad->LastTimeTouched = CTimer::GetTimeInMilliseconds();

	if(++pad->iCurrHornHistory >= CPad::HORNHISTORY_SIZE)
		pad->iCurrHornHistory = 0;

	pad->bHornHistory[pad->iCurrHornHistory] = pad->GetHorn();

	for(int32 i = CPad::DRUNK_STEERING_BUFFER_SIZE - 2; i >= 0; i--)
		pad->SteeringLeftRightBuffer[i + 1] = pad->SteeringLeftRightBuffer[i];

	if(!CPad::bDisplayNoControllerMessage)
		CGame::bDemoMode = false;

	if(pad->JustOutOfFrontend != 0)
		--pad->JustOutOfFrontend;
}

static void
GcReportStickSample(int padIndex, CPad *pad, s8 sx, s8 sy, s8 csx, s8 csy)
{
	static const bool kGcPadTraceEnabled = false;
	static const uint32 kGcPadTraceSampleInterval = 24;
	if(!kGcPadTraceEnabled)
		return;

	if(padIndex != 0 || FrontEndMenuManager.m_bMenuActive)
		return;

	static s8 s_prevSx = 0;
	static s8 s_prevSy = 0;
	static s8 s_prevCsx = 0;
	static s8 s_prevCsy = 0;
	static int16 s_prevWalkX = 0;
	static int16 s_prevWalkY = 0;
	static int16 s_prevCamX = 0;
	static int16 s_prevCamY = 0;
	static uint32 s_seq = 0;
	const int16 walkX = pad->GetPedWalkLeftRight();
	const int16 walkY = pad->GetPedWalkUpDown();
	const int16 camX = pad->GetCarGunLeftRight();
	const int16 camY = pad->GetCarGunUpDown();
	const int16 rightX = pad->NewState.RightStickX;
	const int16 rightY = pad->NewState.RightStickY;
	const bool rawActive = Abs((int32)sx) > 24 || Abs((int32)sy) > 24 ||
		Abs((int32)csx) > 24 || Abs((int32)csy) > 24;
	const bool cStickActive = Abs((int32)csx) > 1 || Abs((int32)csy) > 1 ||
		Abs((int32)rightX) > 1 || Abs((int32)rightY) > 1;
	const bool traceActive = rawActive || cStickActive;
	const bool changed = Abs((int32)sx - (int32)s_prevSx) > 8 ||
		Abs((int32)sy - (int32)s_prevSy) > 8 ||
		Abs((int32)csx - (int32)s_prevCsx) > 6 ||
		Abs((int32)csy - (int32)s_prevCsy) > 6 ||
		Abs((int32)walkX - (int32)s_prevWalkX) > 8 ||
		Abs((int32)walkY - (int32)s_prevWalkY) > 8 ||
		Abs((int32)camX - (int32)s_prevCamX) > 6 ||
		Abs((int32)camY - (int32)s_prevCamY) > 6;

	// EXI/OSReport logging is expensive on Wii. Keep per-frame pad tracing from
	// turning camera-debug sessions into pacing artifacts: log on meaningful
	// changes, otherwise only sample a held stick periodically.
	if(!traceActive || (!changed && (s_seq % kGcPadTraceSampleInterval) != 0)){
		s_seq++;
		return;
	}

	printf("[GC-PAD] seq=%u mode=%d dis=0x%03X raw=(%d,%d) craw=(%d,%d) scaled=(%d,%d) rstick=(%d,%d) cam=(%d,%d) walk=(%d,%d)\n",
		s_seq, pad->GetMode(), (unsigned int)pad->DisablePlayerControls,
		(int)sx, (int)sy, (int)csx, (int)csy,
		(int)pad->NewState.LeftStickX, (int)pad->NewState.LeftStickY,
		(int)rightX, (int)rightY, (int)camX, (int)camY,
		(int)walkX, (int)walkY);
	s_prevSx = sx;
	s_prevSy = sy;
	s_prevCsx = csx;
...
static inline bool
PadBlocksCameraAxisInput(const CPad *pad)
{
	uint16 blocked = pad->DisablePlayerControls;
	blocked &= ~PLAYERCONTROL_CAMERA;
	return blocked != PLAYERCONTROL_ENABLED;
}
#endif
...
static CVehicle*
CreateCheatVehicle(int32 model)
{
#ifdef FIX_BUGS
	const uint8 createdBy = RANDOM_VEHICLE;
#else
	const uint8 createdBy = MISSION_VEHICLE;
#endif

	if(CModelInfo::IsBoatModel(model))
		return new CBoat(model, createdBy);
	if(CModelInfo::IsBikeModel(model))
		return new CBike(model, createdBy);
	...
	return new CAutomobile(model, createdBy);
}
...
void VehicleCheat(int model)
{
	...
	if (CStreaming::ms_aInfoForModel[model].m_loadState == STREAMSTATE_LOADED) {
		...
		CVehicle* vehicle = CreateCheatVehicle(model);
		if (vehicle != nil) {
...
// ▼▼▼ 核心修改：用 #ifndef GAMECUBE 把所有 PC 鼠标逻辑包起来 ▼▼▼
#if !GX_CONSOLE
...
void CPad::UpdatePads(void)
{
	...
	for (int p = 0; p < 2; p++) {
		...
		s8  csy   = PAD_SubStickY(p);
		u8  trigL = PAD_TriggerL(p);
		u8  trigR = PAD_TriggerR(p);
		const bool menuActive = FrontEndMenuManager.m_bMenuActive;

		// libogc reports up as positive; reVC expects negative Y like the PC pad path.
		pad->NewState.LeftStickX  = GcScaleStickAxis(sx,  false);
		pad->NewState.LeftStickY  = GcScaleStickAxis(sy,  true);
		pad->NewState.RightStickX = GcScaleStickAxis(csx, false);
		pad->NewState.RightStickY = GcScaleStickAxis(csy, true);

		// D-Pad
		if (held & PAD_BUTTON_UP)    pad->NewState.DPadUp    = 255;
		if (held & PAD_BUTTON_DOWN)  pad->NewState.DPadDown  = 255;
		if (held & PAD_BUTTON_LEFT)  pad->NewState.DPadLeft  = 255;
		if (held & PAD_BUTTON_RIGHT) pad->NewState.DPadRight = 255;

		if(menuActive) {
			if (held & PAD_BUTTON_A) pad->NewState.Cross = 255;     // A = confirm
			if (held & PAD_BUTTON_B) pad->NewState.Triangle = 255;  // B = back
			if (held & PAD_BUTTON_X) pad->NewState.Square = 255;
			if (held & PAD_BUTTON_Y) pad->NewState.Triangle = 255;
		...
		// Modern GC gameplay layout: A sprint/confirm, B jump, X attack, Y enter/exit.
		if (held & PAD_BUTTON_A) pad->NewState.Cross      = 255;
		if (held & PAD_BUTTON_B) pad->NewState.Square     = 255;
		if (held & PAD_BUTTON_X) pad->NewState.Circle     = 255;
		if (held & PAD_BUTTON_Y) pad->NewState.Triangle   = 255;
...
#endif
		GcReportStickSample(p, pad, sx, sy, csx, csy);
		GcFinishPadFrame(pad);
	}
	return;
#else
...
#else
// ▼▼▼ 核心修改：GameCube 平台不调用老旧的 CapturePad ▼▼▼
#if !GX_CONSOLE
    CapturePad(0);
#endif
// ▲▲▲ GAMECUBE 屏蔽结束 ▲▲▲
...
{
	if ( PadBlocksCameraAxisInput(this) )
		return 0;
...
#if GX_CONSOLE
	return GcActionDown(this, GCA_LOOK_BEHIND);
#endif
	return !!(NewState.RightShoulder2 && NewState.LeftShoulder2);
}
...
#if GX_CONSOLE
	return GcActionAmount(NewState, GCA_HAND_BRAKE);
#endif
	switch (CURMODE)
	{
		case 0:
...
#if GX_CONSOLE
	return GcActionDown(this, GCA_ENTER_EXIT);
#endif
	switch (CURMODE)
	{
		case 0:
...
#if GX_CONSOLE
	return GcActionJustDown(this, GCA_WEAPON_LEFT);
#endif
...
#if GX_CONSOLE
	return GcActionJustDown(this, GCA_WEAPON_RIGHT);
#endif
...
#if GX_CONSOLE
	return GcActionDown(this, GCA_TARGET);
#endif
	switch (CURMODE)
	{
		case 0:
...
#if GX_CONSOLE
	return GcActionJustDown(this, GCA_TARGET);
#endif
	switch (CURMODE)
	{
		case 0:
...
int16 CPad::LookAroundLeftRight(void)
{
	...
	if(Abs(axis) <= 24.0f)
		return 0;
	float sign = axis < 0.0f ? -1.0f : 1.0f;
	float normal = (Abs(axis) - 24.0f) / (128.0f - 24.0f);
	normal = Clamp(normal, 0.0f, 1.0f);
	return (int16)(sign * normal * normal * 127.0f);
#else
	float axis = GetPad(0)->NewState.RightStickX;
...
int16 CPad::LookAroundUpDown(void)
{
	...
	float normal = (Abs((float)axis) - 24.0f) / (128.0f - 24.0f);
	normal = Clamp(normal, 0.0f, 1.0f);
	return (int16)(sign * normal * normal * 127.0f);
#else
	if ( Abs(axis) > 85 && !GetLookBehindForPed() )
