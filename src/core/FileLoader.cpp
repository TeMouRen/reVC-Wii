...
static char
txdMergeLowerAscii(char c)
{
	return (c >= 'A' && c <= 'Z') ? (char)(c - 'A' + 'a') : c;
}

static bool
txdMergeContainsNoCase(const char *name, const char *needle)
{
	...
	for(const char *p = name; *p; p++){
		const char *a = p;
		const char *b = needle;
		while(*a && *b && txdMergeLowerAscii(*a) == txdMergeLowerAscii(*b)){
			a++;
			b++;
		}
		if(*b == '\0')
...
static bool
shouldTraceMergedVegetationTexture(const char *name)
{
	...
	return strcmp(name, "kbtree4_test") == 0 ||
	       ...
	       strcmp(name, "kbpavementgrass256") == 0 ||
	       txdMergeContainsNoCase(name, "tree") ||
	       txdMergeContainsNoCase(name, "leaf") ||
	       txdMergeContainsNoCase(name, "foliage") ||
	       txdMergeContainsNoCase(name, "plant") ||
	       txdMergeContainsNoCase(name, "grass");
}
	...
	for(line = LoadLine(fd); line; line = LoadLine(fd)){
		...
		if(strncmp(line, "IMAGEPATH", 9) == 0){
			...
			if(!objectsLoaded){
				...
				printf("[GC-LOAD] Calling CStreaming::Init...\n");
				CObjectData::Initialise("DATA\\OBJECT.DAT");
				printf("[GC-LOAD] OBJECT.DAT done, calling CStreaming::Init...\n");
				CStreaming::Init();
				POP_MEMID();
...
CFileLoader::LoadTexDictionary(const char *filename)
{
	...
	if(stream){
		...
		if(RwStreamFindChunk(stream, rwID_TEXDICTIONARY, nil, nil)){
			CTxdStore::SetLoadingTxdContext(-2, baseName);
			txd = RwTexDictionaryGtaStreamRead(stream);
			CTxdStore::ClearLoadingTxdContext();
		}
...
#ifdef RW_BIG_ENDIAN
// Little-endian float/int readers for COLL collision data
static inline float _le_float(const uint8 *p) {
	uint32 raw = ((uint32)p[3] << 24) | ((uint32)p[2] << 16) |
	             ((uint32)p[1] << 8)  |  (uint32)p[0];
	float f; memcpy(&f, &raw, 4); return f;
}
static inline int16 _le_i16(const uint8 *p) {
	return (int16)(((uint16)p[1] << 8) | (uint16)p[0]);
}
static inline int32 _le_i32(const uint8 *p) {
	return ((int32)p[3] << 24) | ((int32)p[2] << 16) |
	       ((int32)p[1] << 8)  |  (int32)p[0];
}
#endif
...
#ifdef RW_BIG_ENDIAN
		// COLL files are little-endian PC format; byteswap for big-endian GC
		header.ident = ((header.ident & 0xFF) << 24) |
		               ((header.ident & 0xFF00) << 8) |
		               ((header.ident & 0xFF0000) >> 8) |
		               ((header.ident >> 24) & 0xFF);
		header.size  = ((header.size & 0xFF) << 24) |
		               ((header.size & 0xFF00) << 8) |
		               ((header.size & 0xFF0000) >> 8) |
		               ((header.size >> 24) & 0xFF);
#endif
		if(header.ident != 'LLOC'){
			debug("[COL-BE] Pointer misaligned (ident=0x%08X), starting recovery scan...\n",
			      header.ident);
			// Seek back 7 bytes: we may have overshot by up to 3 bytes
			// (e.g. read "OLL" + next garbage byte as ident)
			CFileMgr::Seek(fd, -7, SEEK_CUR);

			uint8 byte;
			int match = 0;
			const char magic[] = "COLL";
			bool recovered = false;
			int scanned = 0;
#ifdef RW_BIG_ENDIAN
			// Helper: swap LE uint32 → native (big-endian GC)
#define _REC_SWAP32(x) (((x) & 0xFF) << 24) | (((x) & 0xFF00) << 8) | \
                        (((x) & 0xFF0000) >> 8) | (((x) >> 24) & 0xFF)
#else
#define _REC_SWAP32(x) (x)
#endif

			while(CFileMgr::Read(fd, (char*)&byte, 1) == 1){
				scanned++;
				if(byte == (uint8)magic[match]){
					match++;
					if(match == 4){
						// Found "COLL" magic, read size field
						if(CFileMgr::Read(fd, (char*)&header.size, sizeof(header.size))
						   == sizeof(header.size)){
							header.ident = 0x4C4C4F43; // 'LLOC' on big-endian
							header.size = _REC_SWAP32(header.size);
							recovered = true;
							debug("[COL-BE] Recovery OK after %d bytes, resuming\n", scanned);
						}
						break;
					}
				} else {
					match = (byte == (uint8)magic[0]) ? 1 : 0;
				}
				if(scanned > 65536) break; // safety: don't scan forever
			}
#undef _REC_SWAP32
			if(!recovered){
				debug("[COL-BE] Recovery failed after %d bytes, stopping\n", scanned);
				break;
			}
		}
		CFileMgr::Read(fd, (char*)work_buff, header.size);
		memcpy(modelname, work_buff, 24);
		// Diagnostic: log each COLL entry to identify size drift
		debug("[COL-BE] name=%.24s size=%u\n", modelname, header.size);
		...
		// GC: align to 4-byte boundary after each COL entry to prevent misalignment
		if (header.size & 3) {
			CFileMgr::Seek(fd, 4 - (header.size & 3), SEEK_CUR);
		}	}
...
#ifdef RW_BIG_ENDIAN
		// COLL files are little-endian PC format; byteswap for big-endian GC
		modelsize = ((header->size & 0xFF) << 24) |
		            ((header->size & 0xFF00) << 8) |
		            ((header->size & 0xFF0000) >> 8) |
		            ((header->size >> 24) & 0xFF);
		if((((header->ident & 0xFF) << 24) |
		    ((header->ident & 0xFF00) << 8) |
		    ((header->ident & 0xFF0000) >> 8) |
		    ((header->ident >> 24) & 0xFF)) != 'LLOC')
#else
		modelsize = header->size;
		if(header->ident != 'LLOC')
#endif
		{
			// Misalignment recovery — scan buffer for next "COLL" magic
			// (same logic as file-based LoadCollisionFile above)
			uint8 *scan = buffer + 1;
			int match = 0, scanned = 0;
			bool recovered = false;
			while(scanned < 65536 && scan < buffer + size){
				scanned++;
				if(*scan == (uint8)'C') match = 1;
				else if(match == 1 && *scan == (uint8)'O') match = 2;
				else if(match == 2 && *scan == (uint8)'L') match = 3;
				else if(match == 3 && *scan == (uint8)'L'){
					header = (ColHeader*)(scan - 3);
					if((uint8*)(header) + 8 <= buffer + size){
#ifdef RW_BIG_ENDIAN
						modelsize = ((header->size & 0xFF) << 24) |
						            ((header->size & 0xFF00) << 8) |
						            ((header->size & 0xFF0000) >> 8) |
						...
						modelsize = header->size;
#endif
						size -= (int)((uint8*)header - buffer);
						buffer = (uint8*)header;
						recovered = true;
					}
					break;
				}else match = (*scan == (uint8)'C') ? 1 : 0;
				scan++;
			}
			if(!recovered) return true; // no more COLL data, done
		}
		if(modelsize < 24)
			return false;
		uint32 entrySize = 8 + ((modelsize + 3) & ~3);
		if(size < entrySize)
			return false;
...
#ifdef RW_BIG_ENDIAN
#define LE_FLOAT(p) _le_float(p)
#define LE_I16(p)   _le_i16(p)
#define LE_I32(p)   _le_i32(p)
#else
#define LE_FLOAT(p) (*(float*)(p))
#define LE_I16(p)   (*(int16*)(p))
#define LE_I32(p)   (*(int32*)(p))
#endif

	model.boundingSphere.radius  = LE_FLOAT(buf);
	model.boundingSphere.center.x = LE_FLOAT(buf+4);
	model.boundingSphere.center.y = LE_FLOAT(buf+8);
	model.boundingSphere.center.z = LE_FLOAT(buf+12);
	model.boundingBox.min.x = LE_FLOAT(buf+16);
	model.boundingBox.min.y = LE_FLOAT(buf+20);
	model.boundingBox.min.z = LE_FLOAT(buf+24);
	model.boundingBox.max.x = LE_FLOAT(buf+28);
	model.boundingBox.max.y = LE_FLOAT(buf+32);
	model.boundingBox.max.z = LE_FLOAT(buf+36);
	model.numSpheres = LE_I16(buf+40);
	buf += 44;
	if(model.numSpheres > 0){
		...
		for(i = 0; i < model.numSpheres; i++){
			CVector sp_center;
			sp_center.x = LE_FLOAT(buf+4);
			sp_center.y = LE_FLOAT(buf+8);
			sp_center.z = LE_FLOAT(buf+12);
			model.spheres[i].Set(LE_FLOAT(buf), sp_center, buf[16], buf[17]);
			buf += 20;
		}
	}else
		model.spheres = nil;

	model.numLines = LE_I16(buf);
	buf += 4;
	if(model.numLines > 0){
	...
	model.numBoxes = LE_I16(buf);
	buf += 4;
	if(model.numBoxes > 0){
					model.boxes = (CColBox*)RwMalloc(model.numBoxes*sizeof(CColBox));
		REGISTER_MEMPTR(&model.boxes);
		for(i = 0; i < model.numBoxes; i++){
			CVector bx_min, bx_max;
			bx_min.x = LE_FLOAT(buf);   bx_min.y = LE_FLOAT(buf+4);  bx_min.z = LE_FLOAT(buf+8);
			bx_max.x = LE_FLOAT(buf+12); bx_max.y = LE_FLOAT(buf+16); bx_max.z = LE_FLOAT(buf+20);
			model.boxes[i].Set(bx_min, bx_max, buf[24], buf[25]);
			buf += 28;
		}
	}else
		model.boxes = nil;

	int32 numVertices = LE_I16(buf);
	buf += 4;
	if(numVertices > 0){
					model.vertices = (CompressedVector*)RwMalloc(numVertices*sizeof(CompressedVector));
		REGISTER_MEMPTR(&model.vertices);
		for(i = 0; i < numVertices; i++){
			model.vertices[i].Set(LE_FLOAT(buf), LE_FLOAT(buf+4), LE_FLOAT(buf+8));
#if 0
...
", modelname);
	...
	model.numTriangles = LE_I16(buf);
	buf += 4;
	if(model.numTriangles > 0){
					model.triangles = (CColTriangle*)RwMalloc(model.numTriangles*sizeof(CColTriangle));
		REGISTER_MEMPTR(&model.triangles);
		for(i = 0; i < model.numTriangles; i++){
			model.triangles[i].Set(LE_I32(buf), LE_I32(buf+4), LE_I32(buf+8), buf[12]);
			buf += 16;
		}
	}else
		model.triangles = nil;
...
{
	if(texture && shouldTraceMergedVegetationTexture(texture->name))
		printf("[TXD-MERGE] tex=%s srcDict=%p dstDict=%p raster=%p plat=%d %dx%d\n",
		       texture->name,
		       texture->dict,
		       pData,
		       texture->raster,
		       texture->raster ? texture->raster->platform : -1,
		       texture->raster ? texture->raster->width : 0,
		       texture->raster ? texture->raster->height : 0);
...
{
	if(dst && src)
		printf("[TXD-MERGE] begin dst=%p src=%p current=%p\n",
		       dst, src, RwTexDictionaryGetCurrent());
	RwTexDictionaryForAllTextures(src, MoveTexturesCB, dst);
	if(dst && src)
		printf("[TXD-MERGE] end dst=%p src=%p\n", dst, src);
}
