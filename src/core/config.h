...
enum Config {
	...
	// Pool sizes
	NUMPTRNODES = 120000,	// GC: 50000 exhausted during world load
	NUMENTRYINFOS = 3200,
	NUMPEDS = 140,
	NUMVEHICLES = 110,
	NUMBUILDINGS = 7000,
...
#endif
#ifndef WII_STREAM_LIFECYCLE_AUDIT
#define WII_STREAM_LIFECYCLE_AUDIT 0
#endif
#ifndef WII_STREAM_BIG_BUILDING_PROBE
#define WII_STREAM_BIG_BUILDING_PROBE 0
#endif
...
//#ifdef GAMECUBE
//#define USE_CUSTOM_ALLOCATOR		// GC: CMemoryHeap fixed-pool allocator → zero fragmentation
//#endif
...
#if 1 // was: #ifndef GAMECUBE
#define MORE_LANGUAGES            // Add more translations to the game
#ifdef GAMECUBE
#define CHINESE_FONT              // GC Chinese font (wm_vcchs)
#endif
#endif
...
// [GC-FIX] GameCube 使用 LIBRW+RW_NULL，但：
//   EXTENDED_PIPELINES → 需要 neo/neo.txd（不存在）+ CustomPipeInit 真实实现
//   SCREEN_DROPLETS    → 需要 D3D/GL 后备缓冲区
//   NEW_RENDERER       → 需要完整 GPU 管线
//   EXTENDED_COLOURFILTER → 依赖 NEW_RENDERER
// 全部在 GC 上禁用
#ifdef GAMECUBE
...
#undef NEW_RENDERER
#undef DEBUGMENU        // [GC-FIX] 新增：GC 无调试菜单，防止 SETTWEAKPATH/TWEAKBOOL 未定义报错
#undef PC_PLAYER_CONTROLS     // GC: no keyboard/mouse
#endif
