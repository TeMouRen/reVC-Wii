...
{
	HeapBlockDesc *block = GetDescFromHeapPointer(ptr);
	// GC: sanity-check that ptr is actually within our managed heap
	MEMORYHEAP_ASSERT_MESSAGE(block >= m_start && block <= m_end,
		"Free: pointer outside managed heap (malloc/new mismatch?)");
