...
#ifdef WII
	struct WiiAlphaStats
	{
		uint32 culledOnInsert;
		uint32 insertedEntities;
		uint32 insertedUnderwater;
		uint32 insertedBuildings;
		uint32 culledOnRender;
		uint32 renderedDistanceFade;
		uint32 renderedSorted;
		uint32 renderedBoatAtomics;
	};
#endif
...
#ifdef WII
	static void ResetWiiAlphaStats(void);
	static void GetWiiAlphaStats(WiiAlphaStats &stats);
#endif
