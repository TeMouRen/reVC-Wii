...
GeometryListStreamRead1(RwStream *stream, rpGeometryList *geomlist)
{
	...
	if(!RwStreamFindChunk(stream, rwID_STRUCT, &size, &version))
		return nil;
	assert(size == 4);
	if(((rw::Stream*)stream)->read32(&numGeoms, 4) != 4)
		return nil;
...
ClumpAtomicStreamRead(RwStream *stream, rwFrameList *frmList, rpGeometryList *geomList)
{
	...
	if(((rw::Stream*)stream)->read32(&a, size) != size)
		return nil;
...
RpClumpGtaStreamRead1(RwStream *stream)
{
	...
	if(version >= 0x33000){
		assert(size == 12);
		if(((rw::Stream*)stream)->read32(&gClumpInfo, 12) != 12)
			return false;
	}else{
		assert(size == 4);
		if(((rw::Stream*)stream)->read32(&gClumpInfo, 4) != 4)
			return false;
	}
