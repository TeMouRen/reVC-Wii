...
{
#if defined(GAMECUBE)
    // GameCube 的 VFS 自带大小写不敏感解析，直接跳过耗时的遍历
    return nil;
#endif
	...
	while (c = strsep(&p, "/\\"))
	{
		...
		while (e = readdir(d))
		{
			if (strcasecmp(c, e->d_name) == 0)
			{
				...
				// Either it wasnt a folder, or permission error, I/O error etc.
				if (!d) {
					cantProceed = true;
				}
...
// =========================================================
// GameCube 平台存根 (Stubs) - 解决链接器找不到全局符号的问题
// =========================================================
#ifdef GAMECUBE
#include <ogc/lwp_watchdog.h> // 提供 gettime() 和 ticks_to_millisecs()

// 1. 欺骗系统版本检测 (匹配头文件的 long)
long _dwOperatingSystemVersion = 0;

// 2. 欺骗分辨率/显示模式选择器
int32 _psSelectScreenVM = 0;

// 3. 欺骗流媒体引擎，告诉它我们有多少物理内存 (GameCube 约 24MB 主存)
uint32 _dwMemAvailPhys = 64 * 1024 * 1024;

// 4. 游戏全局状态变量 (匹配头文件的 RwUInt32)
RwUInt32 gGameState = 0;

// 5. 平台原生定时器 (极其重要：将 libogc 的硬件时钟转换为毫秒，供引擎计算帧数)
extern "C" uint32 psTimer(void)
{
