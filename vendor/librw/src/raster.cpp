...
static char gRasterConvertDebugName[32];

void
debugSetCurrentConvertingTextureName(const char *name)
{
	if(name == nil){
		gRasterConvertDebugName[0] = '\0';
		return;
	}
	strncpy(gRasterConvertDebugName, name, sizeof(gRasterConvertDebugName));
	gRasterConvertDebugName[sizeof(gRasterConvertDebugName) - 1] = '\0';
}

const char*
debugGetCurrentConvertingTextureName(void)
{
	return gRasterConvertDebugName[0] ? gRasterConvertDebugName : nil;
}

static char
rasLowerAscii(char c)
{
	return (c >= 'A' && c <= 'Z') ? (char)(c - 'A' + 'a') : c;
}

static bool
rasNameContainsNoCase(const char *name, const char *needle)
{
	...
	for(const char *p = name; *p; p++){
		const char *a = p;
		const char *b = needle;
		while(*a && *b && rasLowerAscii(*a) == rasLowerAscii(*b)){
			a++;
			b++;
		}
		if(*b == '\0')
...
static bool
shouldLogFocusedRasterConvert(const char *name)
{
	...
	return strcmp(name, "kbtree4_test") == 0 ||
	       ...
	       rasNameContainsNoCase(name, "foliage") ||
	       rasNameContainsNoCase(name, "grass");
}
...
logFocusedRasterImageSummary(const char *name, const Image *img)
{
	...
	const int sampleCount = img->width * img->height;
	int alphaZero = 0;
	int alphaMid = 0;
	int alphaFull = 0;
	uint32 hash = 2166136261u;

	const uint8 *p = img->pixels;
	for(int i = 0; i < sampleCount; i++, p += 4){
		const uint8 a = p[3];
		if(a == 0)
			alphaZero++;
		else if(a == 255)
	...
	printf("[RASTER-CONVERT-PIX] tex=%s img=%dx%d depth=%d stride=%d a0=%d amid=%d a255=%d hash=%08X p0=%02X%02X%02X%02X\n",
	       name,
	       img->width,
	       img->height,
	       img->depth,
...
{
	using namespace rw;
	const char *debugName = debugGetCurrentConvertingTextureName();
	...
	if(img == nil){
		printf("[RASTER-CONVERT] toImage failed platform=%d format=0x%x size=%dx%d\n",
		       ras->platform, ras->format, ras->width, ras->height);
		return nil;
	}
	...
	if(newras == nil){
		printf("[RASTER-CONVERT] create failed size=%dx%d depth=%d format=0x%x\n",
		       width, height, depth, format);
		img->destroy();
		return nil;
	}
	if(newras->setFromImage(img) == nil){
		printf("[RASTER-CONVERT] setFromImage failed size=%dx%d depth=%d format=0x%x\n",
		       width, height, depth, format);
		img->destroy();
		newras->destroy();
		return nil;
	...
	if(dstNumLevels <= 1 && srcNumLevels > 1){
		printf("[RASTER-CONVERT] dropping %d extra mip level(s) from platform=%d -> %d for %dx%d texture\n",
		       srcNumLevels - 1, ras->platform, newras->platform, width, height);
	}
	int numLevels = srcNumLevels < dstNumLevels ? srcNumLevels : dstNumLevels;
	for(int i = 1; i < numLevels; i++){
		ras->lock(i, Raster::LOCKREAD);
		img = ras->toImage();
		if(img == nil){
			printf("[RASTER-CONVERT] mip toImage failed level=%d\n", i);
			newras->destroy();
			return nil;
		}
		// TODO: maybe don't *always* do this?
		...
		if(newras->setFromImage(img) == nil){
			printf("[RASTER-CONVERT] mip setFromImage failed level=%d\n", i);
			img->destroy();
			newras->unlock(i);
			ras->unlock(i);
			newras->destroy();
