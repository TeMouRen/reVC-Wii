...
Atomic::defaultRenderCB(Atomic *atomic)
{
#if defined(GAMECUBE) && !defined(WII)
	{
		...
		if (_cbCount < 10) {
			ObjPipeline *p = atomic->getPipeline();
			Geometry *g = atomic->geometry;
			uint32 geoFlags = g ? (uint32)g->flags : 0;
			uint32 verts = g ? (uint32)g->numVertices : 0;
			uint32 meshFlags = (g && g->meshHeader) ? (uint32)g->meshHeader->flags : 0;
			uint32 pluginID = p ? (uint32)p->pluginID : 999;
			uint32 pluginData = p ? (uint32)p->pluginData : 999;
			printf("[ATOMIC-RENDER] #%d pipe=%p platform=%u pluginID=%u pluginData=%u geo=%p gflags=0x%x verts=%u meshFlags=0x%x\n",
			       _cbCount, (void*)p,
			       p ? (unsigned)p->platform : 999,
			       (unsigned)pluginID,
			       (unsigned)pluginData,
			       (void*)g, (unsigned)geoFlags, (unsigned)verts, (unsigned)meshFlags);
		}
		_cbCount++;
	}
#endif
