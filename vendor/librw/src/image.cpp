...
// DXT data on disk is always little-endian. On BE platforms (GameCube)
// direct pointer casts would swap bytes; these helpers reconstruct
// the correct value byte-by-byte. On LE platforms the compiler folds
// them into a single load.
static inline uint32 readLE16(const uint8 *p) {
...
static inline uint64 readLE64(const uint8 *p) {
...
decompressDXT1(uint8 *adst, int32 w, int32 h, uint8 *src)
{
	...
	for(int32 j = 0; j < w*h/2; j += 8){
		/* calculate colors */
		uint32 col0 = readLE16(&src[j+0]);
		uint32 col1 = readLE16(&src[j+2]);
		...
		/* make index list */
		uint32 indices = readLE32(&src[j+4]);
...
decompressDXT3(uint8 *adst, int32 w, int32 h, uint8 *src)
{
	...
	for(int32 j = 0; j < w*h; j += 16){
		/* calculate colors */
		uint32 col0 = readLE16(&src[j+8]);
		uint32 col1 = readLE16(&src[j+10]);
		c[0][0] = ((col0>>11) & 0x1F)*0xFF/0x1F;
		c[0][1] = ((col0>> 5) & 0x3F)*0xFF/0x3F;
		c[0][2] = ( col0      & 0x1F)*0xFF/0x1F;
		...
		/* make index list */
		uint32 indices = readLE32(&src[j+12]);
		for(int32 k = 0; k < 16; k++){
			idx[k] = indices & 0x3;
			indices >>= 2;
		}
		uint64 alphas = readLE64(&src[j+0]);
		for(int32 k = 0; k < 16; k++){
			a[k] = (alphas & 0xF)*17;
			alphas >>= 4;
		}
...
decompressDXT5(uint8 *adst, int32 w, int32 h, uint8 *src)
{
	...
	for(int32 j = 0; j < w*h; j += 16){
		...
		// only 6 indices
		uint64 alphas = readLE64(&src[j+2]);
		for(int32 k = 0; k < 16; k++){
			aidx[k] = alphas & 0x7;
			alphas >>= 3;
		}
