...
static const char*
debugRasterOwnerName(Raster *raster)
{
	if(raster == nil || raster->parent == nil)
		return nil;
	Texture *tex = (Texture*)raster->parent;
...
rasterToImage(Raster *raster)
{
	...
	if(natras->customFormat){
		int w = raster->width;
		int h = raster->height;
		const char *ownerName = debugRasterOwnerName(raster);
		...
		if(ownerName && isKeyVegetationDebugTexture(ownerName)){
			unsigned aLo = 0, aMid = 0, aHi = 0;
			unsigned rMin = 255, gMin = 255, bMin = 255;
			unsigned rMax = 0, gMax = 0, bMax = 0;
			unsigned long rSum = 0, gSum = 0, bSum = 0, vis = 0;
			for(int yy = 0; yy < image->height; yy++){
				const uint8 *row = image->pixels + yy * image->stride;
			...
			const uint8 *c = image->pixels + (image->height/2) * image->stride + (image->width/2) * 4;
			printf("[D3D-DECODEPROBE] %s: fmt=0x%08X %dx%d aHist=%u/%u/%u vis=%lu rgbMin=%u,%u,%u rgbMax=%u,%u,%u rgbAvg=%u,%u,%u center=%u,%u,%u,%u\n",
			       ownerName,
			       (unsigned)natras->format,
			       image->width,
			       image->height,
			       aLo, aMid, aHi,
			       vis,
			       rMin, gMin, bMin,
			       rMax, gMax, bMax,
			       vis ? (unsigned)(rSum / vis) : 0u,
			       vis ? (unsigned)(gSum / vis) : 0u,
			       vis ? (unsigned)(bSum / vis) : 0u,
