...
{
#if defined(BIGENDIAN) || defined(RW_BIG_ENDIAN)
...
findChunk(Stream *s, uint32 type, uint32 *length, uint32 *version)
{
	...
	while(readChunkHeaderInfo(s, &header)){
		...
		if(header.type == 0 && header.length == 0){
			SYS_Report("[FATAL] findChunk: garbage (type=0,len=0) at pos=%u, aborting\n",
				s->tell());
			return false;
		}
		uint32 posBefore = s->tell();
#endif
		s->seek(header.length);
#ifdef GAMECUBE
		if(s->tell() == posBefore && header.length != 0){
			SYS_Report("[FATAL] findChunk: seek stalled at pos=%u len=%d, aborting\n",
				posBefore, header.length);
			return false;
		}
#endif
	}
...
getFileContents(const char *name, uint32 *len)
{
	...
	*len = engine->filefuncs.rwftell(cf);
	engine->filefuncs.rwfseek(cf, 0, SEEK_SET);
		printf("[FILE-LOAD] %s: size=%u attempting alloc...\n", name, (unsigned)*len);
		uint8 *data = (uint8*)rwMalloc(*len, MEMDUR_EVENT);
		if(data == nil){
			printf("[FILE-LOAD] %s: malloc(%u) FAILED — heap exhausted, returning nil\n", name, (unsigned)*len);
