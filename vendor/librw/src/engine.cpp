...
{
	if(s_rwEmergencyReserve){
		Engine::memfuncs.rwfree(s_rwEmergencyReserve);
		s_rwEmergencyReserve = nil;
		printf("[GX-MEM] released rw emergency reserve (%u bytes) due to %s\n",
			(unsigned)s_rwEmergencyReserveSize,
			reason ? reason : "<unknown>");
	}
}

static void
ensureRwEmergencyReserve(void)
{
	if(s_rwEmergencyReserve == nil && s_rwEmergencyReserveSize > 0){
		s_rwEmergencyReserve = Engine::memfuncs.rwmalloc(s_rwEmergencyReserveSize, 0);
		if(s_rwEmergencyReserve)
			printf("[GX-MEM] reserved rw emergency block (%u bytes)\n",
				(unsigned)s_rwEmergencyReserveSize);
	}
}
...
{
	const uint32 kBudgetStep = 2u * 1024u * 1024u;
	const uint32 kBudgetCap = 24u * 1024u * 1024u;
	const uint32 kBudgetFloor = 18u * 1024u * 1024u;
	uint32 oldBudget = gx::texPoolGetSoftBudget();
	uint32 newBudget = oldBudget;

	if(newBudget > kBudgetCap)
		newBudget = kBudgetCap;
	if(newBudget > kBudgetFloor + kBudgetStep)
		newBudget -= kBudgetStep;
	else if(newBudget > kBudgetFloor)
		newBudget = kBudgetFloor;
	if(newBudget > oldBudget)
		newBudget = oldBudget;

	if(newBudget != oldBudget) {
		printf("[GX-MEM] tightening texture budget after %s: %uKB -> %uKB (pool=%uKB/%d tex)\n",
			reason ? reason : "rw OOM",
			(unsigned)(oldBudget / 1024),
			(unsigned)(newBudget / 1024),
			(unsigned)(gx::texPoolTotalBytes() / 1024),
			gx::texPoolCount());
		gx::texPoolSetSoftBudget(newBudget);
	}
...
void *mustmalloc_h(size_t sz, uint32 hint)
{
	...
	if(ret || sz == 0)
		return ret;
#if defined(RW_GX) && defined(GAMECUBE)
	printf("[GX-MEM] rwMalloc(%u, hint=0x%08X) OOM at %s; texture pool=%uKB/%d tex\n",
		(unsigned)sz, (unsigned)hint,
		allocLocation ? allocLocation : "<unknown>",
		(unsigned)(gx::texPoolTotalBytes()/1024), gx::texPoolCount());
	releaseRwEmergencyReserve("rwMalloc OOM");
	ret = Engine::memfuncs.rwmalloc(sz, hint);
	if(ret){
		printf("[GX-MEM] rwMalloc(%u) recovered from emergency reserve, pool=%uKB/%d tex\n",
			(unsigned)sz,
			(unsigned)(gx::texPoolTotalBytes()/1024), gx::texPoolCount());
	...
	ret = Engine::memfuncs.rwmalloc(sz, hint);
	if(ret){
		printf("[GX-MEM] rwMalloc(%u) recovered after budget tighten, pool=%uKB/%d tex\n",
			(unsigned)sz,
			(unsigned)(gx::texPoolTotalBytes()/1024), gx::texPoolCount());
		ensureRwEmergencyReserve();
		return ret;
	}
	for(int attempt = 1; attempt <= s_rwRecoveryShrinkAttempts; attempt++){
		bool shrunk = gx::shrinkLargestTexturePreferHostMemory();
		if(!shrunk)
			shrunk = gx::shrinkLargestTexture();
		if(!shrunk){
			printf("[GX-MEM] rwMalloc recovery: no shrinkable textures on attempt %d\n",
				attempt);
			break;
		}
		ret = Engine::memfuncs.rwmalloc(sz, hint);
		if(ret){
			printf("[GX-MEM] rwMalloc(%u) recovered after %d texture shrink(s), pool=%uKB/%d tex\n",
				(unsigned)sz, attempt,
				(unsigned)(gx::texPoolTotalBytes()/1024), gx::texPoolCount());
			ensureRwEmergencyReserve();
			return ret;
...
void *mustrealloc_h(void *p, size_t sz, uint32 hint)
{
	...
	if(ret || sz == 0)
		return ret;
#if defined(RW_GX) && defined(GAMECUBE)
	printf("[GX-MEM] rwRealloc(%p, %u, hint=0x%08X) OOM at %s; texture pool=%uKB/%d tex\n",
		p, (unsigned)sz, (unsigned)hint,
		allocLocation ? allocLocation : "<unknown>",
		(unsigned)(gx::texPoolTotalBytes()/1024), gx::texPoolCount());
	releaseRwEmergencyReserve("rwRealloc OOM");
	ret = Engine::memfuncs.rwrealloc(p, sz, hint);
	if(ret){
		printf("[GX-MEM] rwRealloc(%u) recovered from emergency reserve, pool=%uKB/%d tex\n",
			(unsigned)sz,
			(unsigned)(gx::texPoolTotalBytes()/1024), gx::texPoolCount());
		ensureRwEmergencyReserve();
		return ret;
	...
	ret = Engine::memfuncs.rwrealloc(p, sz, hint);
	if(ret){
		printf("[GX-MEM] rwRealloc(%u) recovered after budget tighten, pool=%uKB/%d tex\n",
			(unsigned)sz,
			(unsigned)(gx::texPoolTotalBytes()/1024), gx::texPoolCount());
		ensureRwEmergencyReserve();
		return ret;
	...
	for(int attempt = 1; attempt <= s_rwRecoveryShrinkAttempts; attempt++){
		...
		if(!shrunk)
			shrunk = gx::shrinkLargestTexture();
		if(!shrunk){
			printf("[GX-MEM] rwRealloc recovery: no shrinkable textures on attempt %d\n",
				attempt);
			break;
		}
		ret = Engine::memfuncs.rwrealloc(p, sz, hint);
		if(ret){
			printf("[GX-MEM] rwRealloc(%u) recovered after %d texture shrink(s), pool=%uKB/%d tex\n",
				(unsigned)sz, attempt,
				(unsigned)(gx::texPoolTotalBytes()/1024), gx::texPoolCount());
			ensureRwEmergencyReserve();
			return ret;
...
#if defined(RW_GX) && defined(GAMECUBE)
	// Reserve the fallback block only after the allocator table is valid.
	ensureRwEmergencyReserve();
#endif
