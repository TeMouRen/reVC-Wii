                       ...
                       int32 flags, int32 maxKeySize)
{
	...
	if(hier->interpolator == nil){
		RWERROR((ERR_ALLOC, numNodes));
		rwFree(hier);
		return nil;
	}
	...
	if(hier->flags & NOMATRICES){
		...
		if(hier->matricesUnaligned == nil){
			RWERROR((ERR_ALLOC, hier->numNodes*64 + 0xF));
			hier->interpolator->destroy();
			rwFree(hier);
			return nil;
		}
		hier->matrices =
	...
	if(hier->nodeInfo == nil){
		RWERROR((ERR_ALLOC, hier->numNodes*sizeof(HAnimNodeInfo)));
		rwFree(hier->matricesUnaligned);
		hier->interpolator->destroy();
		rwFree(hier);
		return nil;
	}
...
readHAnim(Stream *stream, int32, void *object, int32 offset, int32)
{
	...
	if(numNodes != 0){
		...
		int32 *nodeIDs = rwNewT(int32, numNodes,
			MEMDUR_FUNCTION | ID_HANIM);
		if(nodeFlags == nil || nodeIDs == nil){
			RWERROR((ERR_ALLOC, numNodes*sizeof(int32)));
			rwFree(nodeFlags);
			rwFree(nodeIDs);
			return nil;
		}
