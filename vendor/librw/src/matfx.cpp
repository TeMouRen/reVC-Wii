...
float32 matFXEnvMapIntensity = 1.0f;
...
#ifdef GAMECUBE
	static int s_badPipeCount = 0;
	if(atomic->pipeline && atomic->pipeline->platform != rw::platform &&
	   s_badPipeCount < 16){
		printf("[MATFX-PIPE] assigned pipe=%p platform=%u pluginID=%u pluginData=%u target=%d\n",
		       (void*)atomic->pipeline,
		       (unsigned)atomic->pipeline->platform,
		       (unsigned)atomic->pipeline->pluginID,
		       (unsigned)atomic->pipeline->pluginData,
		       (int)rw::platform);
		s_badPipeCount++;
	}
#endif
